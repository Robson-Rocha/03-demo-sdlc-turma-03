...
public sealed class TrainingUpdateTests
{
    ...
    [Fact]
    public async Task ReturnsBadRequestWhenUpdatedDurationExceedsFourHours()
    {
        using var factory = new TrainingCatalogApiFactory();
        using var client = factory.CreateClient();
        var training = await CreateTraining(client, "2026-09-15");
        var request = new CreateTrainingRequest("C# Avançado", "Tópicos avançados de C#", "2026-09-16", 5);
